...
Linux perf:
  ...
  instructions per cycle (IPC) > 2       ← CPU is busy computing
  cache-miss rate < 1%                   ← data fits in cache
  CPU utilization ~100%

  # Memory-bound signals:
  IPC < 1                                ← CPU stalling on memory
  LLC (L3) cache-miss rate > 5%          ← data doesn't fit in cache
  memory bandwidth near peak              ← DRAM is saturated
  CPU utilization < 100% despite work

Quick rule of thumb:
  Does your working set fit in L3 cache?
    Yes (< 30 MB) → probably compute-bound
    No  (>> 30 MB) → probably memory-bound

  Is your access pattern sequential?
    Yes → memory subsystem can prefetch, less memory-bound
    No (random/pointer-chasing) → memory latency kills you
```

### Optimization Decision Tree

```
Program is slow. What's the bottleneck?
│
├─ perf stat → IPC > 2, CPU 100%
│  └─ COMPUTE-BOUND
│     ├─ Better algorithm? (always check first)
│     ├─ Can SIMD/vectorize the hot loop?
│     ├─ Can reduce precision? (f64 → f32)
│     ├─ Can parallelize across cores? (rayon)
│     └─ Compiler: -O3, LTO, PGO?
│
├─ perf stat → IPC < 1, high cache misses
│  └─ MEMORY-BOUND (latency)
│     ├─ Smaller data types? (u64 → u32)
│     ├─ Cache-friendly layout? (SoA vs AoS)
│     ├─ Sequential access instead of random?
│     ├─ Avoid pointer chasing? (Vec vs LinkedList)
│     └─ Prefetching / tiling?
│
├─ perf stat → IPC < 1, bandwidth saturated
│  └─ MEMORY-BOUND (bandwidth)
│     ├─ Smaller data? (fewer bytes to move)
│     ├─ Compression? (if compute is cheap)
│     ├─ SIMD to process more per load?
│     └─ Fewer passes over data?
│
└─ Neither CPU nor memory saturated?
   └─ Probably I/O-BOUND (disk, network, locks)
      ├─ Disk: async I/O, io_uring, SSD, mmap
      ├─ Network: connection pool, batching, compression
      └─ Locks: sharding, lock-free, read-write lock
```
