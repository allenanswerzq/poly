...
fn new_state() -> AppState {
    let users = vec![
        ...
        User {
            ...
            created_at: "2024-01-07T00:00:00Z".into(),
        },
    ];
    AppState {
        ...
        rt.block_on(async move {
            let app = axum::Router::new()
                ...
                .route(
                    ...
                    axum::routing::get(list_users_paginated),
                )
                // Idempotent create
                ...
                .route(
                    "/api/v1/users-fields",
                    axum::routing::get(list_users_fields),
                )
                .with_state(state);
...
async fn create_payment(
    ...
    if !idempotency_key.is_empty() {
        state
            ...
            .write()
            .unwrap()
            .insert(idempotency_key, payment.clone());
    }
...
async fn batch_get_users(
    ...
    let results: Vec<Value> = body
        .ids
        .iter()
        .map(|id| match users.iter().find(|u| &u.id == id) {
...
fn demo_idempotency() {
    ...
    let resp3: Value = client
        ...
        .unwrap();
    println!(
        "  3rd request: id={} (new key → new payment)",
        resp3["payment"]["id"]
    );
...
fn demo_versioning() {
    ...
    let v1: Value = client
        .get("http://127.0.0.1:9200/api/v1/users")
        .send()
        .unwrap()
        .json()
        .unwrap();
    ...
    let v2: Value = client
        .get("http://127.0.0.1:9200/api/v2/users")
        .send()
        .unwrap()
        .json()
        .unwrap();
...
fn demo_batch() {
    ...
    for id in ["u1", "u3", "u5"] {
        client
            .get(format!("http://127.0.0.1:9200/api/v1/users/{}", id))
            .send()
            .unwrap();
    }
    ...
    let start = Instant::now();
    let resp: Value = client
        .post("http://127.0.0.1:9200/api/v1/users/batch")
...
fn demo_async_job() {
    ...
    // Poll → pending
    let resp: Value = client
        .get(format!("{}/reports/{}", base, job_id))
        .send()
        .unwrap()
        .json()
        .unwrap();
...
fn demo_field_selection() {
    ...
    // Only id,name
    let resp: Value = client
        .get(format!("{}/users-fields?fields=id,name", base))
        .send()
        .unwrap()
        .json()
        .unwrap();
    ...
    // Only email
    let resp: Value = client
        .get(format!("{}/users-fields?fields=email", base))
        .send()
        .unwrap()
        .json()
        .unwrap();
