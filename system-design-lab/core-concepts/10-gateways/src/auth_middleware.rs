...
pub fn demo_auth_middleware() {
    ...
    rt.block_on(async {
        ...
        let auth_gateway = axum::Router::new().route(
            "/api/users/:id",
            axum::routing::get(move |Path(id): Path<u32>, headers: HeaderMap| {
                let keys = keys.clone();
                async move {
                    let api_key = headers
                        .get("x-api-key")
        ...
        let listener = tokio::net::TcpListener::bind("127.0.0.1:9104")
            .await
            .unwrap();
        ...
        let resp = client
            .get("http://127.0.0.1:9104/api/users/1")
            .send()
            .await
            .unwrap();
        println!(
        ...
        let resp = client
            ...
            .unwrap();
        println!(
            "    Valid key:       {} (forwarded to backend)",
            resp.status()
        );
